    ...
    public class NHibernateSessionModule : IHttpModule
    {
        ...
        private void CommitAndCloseSession(object sender, EventArgs e) {
            try {
                ...
                {
                    lastError = (Exception)HttpContext.Current.Items["ERROR"];
                }
                if ( lastError != null)
                {
                    WITLogManager log = WITLogManager.GetInstance("WIT.Common.NHibernate");
                    log.LogError(lastError.Message, lastError);
